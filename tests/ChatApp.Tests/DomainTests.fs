...
module UserHandleTests =
    ...
    let ``UserHandle.create succeeds with valid input`` () =
        ...
        match result with
        | Result.Ok handle -> Assert.Equal("alice-123", UserHandle.value handle)
    ...
    let ``UserHandle.create fails with empty input`` () =
        ...
        match result with
        | Result.Error EmptyUserHandle -> () // Expected
        | Result.Ok handle -> Assert.Fail($"Expected failure but got: {handle}")
        | Result.Error err -> Assert.Fail($"Expected EmptyUserHandle but got: {err}")
    
    [<Fact>]
    let ``UserHandle.create fails with invalid characters`` () =
        let result = UserHandle.create "alice@123"
        match result with
        | Result.Error (InvalidUserHandleChars _) -> () // Expected
        | Result.Ok handle -> Assert.Fail($"Expected failure but got: {handle}")
        | Result.Error err -> Assert.Fail($"Expected InvalidUserHandleChars but got: {err}")
...
module RoomNameTests =
    ...
    let ``RoomName.create succeeds with valid input`` () =
        ...
        match result with
        | Result.Ok room -> Assert.Equal("general-chat", RoomName.value room)
        | Result.Error err -> Assert.Fail($"Expected success but got error: {err}")
    ...
    let ``RoomName.create fails with empty input`` () =
        ...
        match result with
        | Result.Error EmptyRoomName -> () // Expected
        | Result.Ok room -> Assert.Fail($"Expected failure but got: {room}")
        | Result.Error err -> Assert.Fail($"Expected EmptyRoomName but got: {err}")
    
    [<Fact>]
    let ``RoomName.create fails with invalid characters`` () =
        let result = RoomName.create "general chat"  // space is invalid
        match result with
        | Result.Error (InvalidRoomNameChars _) -> () // Expected
        | Result.Ok room -> Assert.Fail($"Expected failure but got: {room}")
        | Result.Error err -> Assert.Fail($"Expected InvalidRoomNameChars but got: {err}")
...
module MessageContentTests =
    ...
    let ``MessageContent.create succeeds with valid input`` () =
        ...
        match result with
        | Result.Ok content -> Assert.Equal("Hello, world!", MessageContent.value content)
        | Result.Error err -> Assert.Fail($"Expected success but got error: {err}")
    ...
    let ``MessageContent.create fails with empty input`` () =
        ...
        match result with
        | Result.Error EmptyMessageContent -> () // Expected
        | Result.Ok content -> Assert.Fail($"Expected failure but got: {content}")
        | Result.Error err -> Assert.Fail($"Expected EmptyMessageContent but got: {err}")
    ...
    let ``MessageContent.create fails with too long input`` () =
        ...
        match result with
        | Result.Error (MessageContentTooLong _) -> () // Expected
        | Result.Ok content -> Assert.Fail($"Expected failure but got: {content}")
        | Result.Error err -> Assert.Fail($"Expected MessageContentTooLong but got: {err}")